...
use gpui::{
    Context, Entity, Hsla, InteractiveElement, IntoElement, ParentElement,
...
pub(crate) fn render(state: &Entity<AppState>, cx: &mut Context<AppView>) -> impl IntoElement {
    ...
    let pills: [(MonitorKind, String, PmIcon, Hsla, bool); 5] = [
        (MonitorKind::Registry, rust_i18n::t!("mon.registry").to_string(), PmIcon::Registry, pal.op_registry, m.registry),
        (MonitorKind::File, rust_i18n::t!("mon.file").to_string(), PmIcon::Filesys, pal.op_file, m.file),
        (MonitorKind::Network, rust_i18n::t!("mon.network").to_string(), PmIcon::Network, pal.op_network, m.network),
        (MonitorKind::Process, rust_i18n::t!("mon.process").to_string(), PmIcon::ProcThread, pal.op_process, m.process),
        (MonitorKind::Profiling, rust_i18n::t!("mon.profiling").to_string(), PmIcon::Perf, pal.op_perf, m.profiling),
    ];
    ...
    h_flex()
        ...
        .children(pills.into_iter().map(move |(kind, label, icon, color, active)| {
            ...
            let dot = div()
                .size(px(7.))
                .rounded_full()
                .bg(if active { accent } else { muted.opacity(0.5) });
