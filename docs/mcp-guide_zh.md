...
- **`process_timeline`** —— 某 PID 的关键活动(有副作用的操作 + 网络),按时间排序、折叠读/查询——快速看"这个进程干了什么"。
- **`event_window`** —— 某个 `seq` 前后的事件(默认同进程),看清它前因后果。
- **`process_tree`** —— 父→子 进程树(只给结构 + 名字)。
- **`list_processes`** —— 进程身份 + 截断的命令行(分页;完整命令行在 `get_process`)。
...
- **响应有服务端上限。** 每个工具结果都被限制(~48 KB);一旦某个请求会超,服务器返回一句
  "请收窄"的提示(用 `group_by` / 过滤 / `get_process`),而不是甩一大堆数据 —— 所以再大的捕获
  也不会撑爆你的上下文。`list_processes` 分页、`process_tree` 只回 pid/名字结构,都是这个原因。
- **要摘要、别要倾倒。** "把写入按路径分组""哪些进程最忙" —— agent 会返回计数而不是几千行原始事件。
